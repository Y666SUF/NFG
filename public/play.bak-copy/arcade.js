import { api, esc, isMobileGameDevice, getSession } from "./shared.js";
...
function mountGame(game, bodyEl, onBalanceChange) {
  ...
  const id = game.id;
  if (id === "nfg_blocks") {
    mountBlocksGame(bodyEl, onWallet);
    return;
  }
  if (id === "nfg_snake_jump") {
    mountJumpGame(bodyEl, onWallet, getSession());
    return;
  }
...
export function initArcade({ onBalanceChange } = {}) {
  ...
  function closeOverlay() {
    if (overlay) {
      overlay.hidden = true;
      overlay.classList.remove("arcade-overlay--blocks", "arcade-overlay--jump");
    }
  ...
  function openGame(game) {
    ...
    overlay.classList.toggle("arcade-overlay--blocks", game.id === "nfg_blocks");
    overlay.classList.toggle("arcade-overlay--jump", game.id === "nfg_snake_jump");
    if (title) title.textContent = game.title === "NFG Blocks" ? "NFG Blocks" : game.title || game.id;
  ...
  async function loadCatalog() {
    ...
    try {
      ...
      const games = (data?.games || []).filter((g) => {
        ...
        if (g.id === "nfg_snake_jump" && !isMobileGameDevice()) return false;
        return true;
      });
