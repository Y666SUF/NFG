...
struct GameAPI {
    ...
    /// Full public card for leaderboard player tap (bets, shield, Dragon Tower hero).
    func fetchPlayerLookup(user: String) async throws -> PlayerLookupResponse {
        let encoded = user.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? user
        let (data, response) = try await GameHTTP.data(from: baseURL.appending(path: "/api/economy/lookup/\(encoded)"))
        guard let http = response as? HTTPURLResponse else {
            throw GameAPIError.serverError("No response")
        }
        ...
        return try JSONDecoder().decode(PlayerLookupResponse.self, from: data)
    }
