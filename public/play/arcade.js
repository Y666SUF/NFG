...
import { JumpVsClient } from "./jump-vs-client.js";
...
export function initArcade({ onBalanceChange }) {
  ...
  async function loadLeaderboards(el) {
    ...
    if (jump?.top?.length) {
      html += `<h3>NFG Jump top</h3><ul class="board-list jump-lb-list">${jump.top
        .slice(0, 5)
        .map((r, i) => `<li class="jump-lb-item">${jumpLeaderboardLine(r, i + 1)}</li>`)
        .join("")}</ul>`;
    }
    el.innerHTML = html;
  }

  function jumpLeaderboardLine(r, rank) {
    const name = esc(r.displayName || r.userId);
    const score = `${Number(r.points || 0).toLocaleString()}m`;
    if (!r.jumpSkinName || r.jumpSkinId === "classic") {
      return `<span class="jump-lb-rank">${rank}.</span> ${name} <span class="jump-lb-score">${score}</span>`;
    }
    const fill = esc(r.jumpSkinFill || "#596ff2");
    const ring = esc(r.jumpSkinRing || "#f2c733");
    const title = esc(r.jumpSkinName);
    return `
      <span class="jump-lb-rank">${rank}.</span>
      <span class="jump-lb-orb" style="--fill:${fill};--ring:${ring}"></span>
      <span class="jump-lb-copy">
        <span class="jump-lb-name">${name}</span>
        <span class="jump-lb-title" style="color:${ring}">${title}</span>
      </span>
      <span class="jump-lb-score">${score}</span>`;
  }
...
function mountMines(ctx) {
  ...
  cashBtn.addEventListener("click", async () => {
    try {
      const res = await play(ctx, "cashout");
      renderGrid(res);
      showToast(res.message || "Cashed out");
    } catch (e) {
      msgEl(root).textContent = e.message;
    }
  });
...
function mountBlocks(ctx) {
  ...
  const mount = mountBlocksGame(host, {
    serverLevel: ctx.status.level || 1,
    sessionPoints: ctx.status.sessionPoints || 0,
    sessionActive: ctx.status.sessionActive,
    rewardPreview: ctx.status.levelRewardPreview,
    onStart: async () => {
      const res = await play(ctx, "start");
      return res;
    },
    onLevelClear: async () => play(ctx, "level_clear"),
    onGameOver: async () => play(ctx, "game_over"),
  });
  return mount;
}

function jumpHooksFromStatus(status) {
  return {
    sessionPoints: status.sessionPoints || 0,
    sessionMilestones: status.sessionMilestones || 0,
    bestHeight: status.bestLevel || 0,
    rewardPreview: status.levelRewardPreview || 3000,
    balance: status.balance ?? 0,
    jumpShop: status.jumpShop || [],
    equippedSkin: status.equippedSkin || "classic",
    ownedSkins: status.ownedSkins || ["classic"],
    skinFill: status.skinFill || "#596ff2",
    skinRing: status.skinRing || "#f2c733",
  };
}
...
function mountJump(ctx) {
  ...
  ctx.container.appendChild(host);
  let vsClient = null;
  let vsMatchSeed = null;
  let vsMatchId = null;
  let vsMode = false;

  function ghostOpponentsFromVs() {
    if (!vsClient) return [];
    const me = getSession().userId;
    return vsClient.opponentList().map((opp, i) => ({
      id: opp.id,
      fill: opp.fill,
      ring: opp.ring,
      x: ((opp.id || "").charCodeAt(0) % 7) * 18 - 54 + i * 12,
      worldY: (opp.height || 0) + 80,
    }));
  }

  function vsRefreshExtra() {
    return {
      vsMode,
      vsMatchSeed,
      vsMatchId,
      vsClient,
      vsLobbyState: vsClient?.snapshot() || null,
      ghostOpponents: ghostOpponentsFromVs(),
    };
  }

  const mount = mountJumpGame(host, {
    ...jumpHooksFromStatus(ctx.status),
    ...vsRefreshExtra(),
    onVsJoin: () => {
      const hooks = jumpHooksFromStatus(ctx.status);
      vsClient = new JumpVsClient({
        skinId: hooks.equippedSkin,
        fill: hooks.skinFill,
        ring: hooks.skinRing,
        onLobbyState: (state) => {
          vsMode = !!vsClient;
          mount.refresh({ ...jumpHooksFromStatus(ctx.status), ...vsRefreshExtra(), vsLobbyState: state });
        },
        onMatchStart: (state) => {
          vsMode = true;
          ...
          mount.refresh({ ...jumpHooksFromStatus(ctx.status), ...vsRefreshExtra(), vsLobbyState: state });
          mount.refresh({ ...jumpHooksFromStatus(ctx.status), ...vsRefreshExtra() });
        },
        onOpponents: () => {
          mount.refresh({ ...jumpHooksFromStatus(ctx.status), ...vsRefreshExtra() });
        },
        onEliminated: (reason) => {
          mount.refresh({
            ...jumpHooksFromStatus(ctx.status),
            ...vsRefreshExtra(),
            vsLobbyState: { ...vsClient.snapshot(), eliminated: true, phase: "match" },
          });
        },
        onMatchEnd: (msg) => {
          vsMode = false;
          ...
          mount.refresh({
            ...jumpHooksFromStatus(ctx.status),
            ...vsRefreshExtra(),
            vsLobbyState: { phase: "results", pot: msg.pot, winnerId: msg.winnerId, players: msg.rankings || [] },
          });
        },
        onError: (err) => {
          mount.refresh({ ...jumpHooksFromStatus(ctx.status), ...vsRefreshExtra() });
    ...
    onStart: async (payload = {}) => {
      const res = await play(ctx, "start", payload);
      mount.refresh({ ...jumpHooksFromStatus(ctx.status), ...vsRefreshExtra() });
      return res;
    },
    onMilestone: async (height) => {
      const res = await play(ctx, "milestone", { height });
      mount.refresh(jumpHooksFromStatus(ctx.status));
      return res;
    },
    onGameOver: async (height) => {
      const res = await play(ctx, "game_over", { height });
      mount.refresh(jumpHooksFromStatus(ctx.status));
      return res;
    },
    onBuySkin: async (itemId) => {
      const res = await play(ctx, "buy", { itemId });
      mount.refresh(jumpHooksFromStatus(ctx.status));
      return res;
    },
    onEquipSkin: async (itemId) => {
      const res = await play(ctx, "equip", { itemId });
      mount.refresh(jumpHooksFromStatus(ctx.status));
      return res;
    },
  });
  const baseDestroy = mount.destroy;
  mount.destroy = () => {
    vsClient?.disconnect();
    baseDestroy();
  };
  Object.defineProperty(mount, "vsClient", {
    get: () => vsClient,
    configurable: true,
  });
  return mount;
