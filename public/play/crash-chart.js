/** Canvas crash chart — storm sky, Patrick Star + Mjolnir flight (play). */
const THEME = {
  accent: "#60a5fa",
  accent2: "#fbbf24",
  danger: "#ef4444",
...
class FlightLayout {
  ...
  flightAngle(mult) {
    ...
    const p0 = this.point(Math.max(1, m - 0.08), { uncapped: true });
...
export class CrashChartRenderer {
  ...
  resize() {
    ...
    document.documentElement.style.setProperty("--chart-h", `${Math.round(h)}px`);
  ...
  draw() {
    ...
    const yMax =
      running || crashAnim ? this.runningYMax || 15 : Math.max(mult * 1.22, 1.5);
    const layout = new FlightLayout(w, h, yMax);
    const crashed = this.phase === "ended" || crashAnim;

    drawSkyScene(ctx, w, h);
    ...
    const pos = this.rocketPosition(layout, mult, running);
    const baseAngle = layout.flightAngle(mult);
    const angle = crashAnim ? baseAngle + 1.55 : baseAngle;
