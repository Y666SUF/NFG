/** Canvas crash chart — galaxy, rocket flight path, iOS-style chrome. */
const THEME = {
  accent: "#4fd1ff",
  accent2: "#7ee7c4",
  danger: "#ff6b6b",
...
export class CrashChartRenderer {
  constructor(canvas, wrap) {
    ...
    if (typeof ResizeObserver !== "undefined") {
      ...
      const scene = wrap.closest(".play-scene");
      if (scene) {
        this.sceneRo = new ResizeObserver(() => this.resize());
        this.sceneRo.observe(scene);
      }
    }
    this.resize();
    this.loop();
  ...
  chartHeight() {
    ...
    return Math.min(380, Math.max(150, Math.round(layoutH * 0.9)));
  }
