...
struct CrashChartView: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            let exhaustIntensity = exhaustIntensity(for: liveMult)
            ...
            ZStack {
                ...
                if crashPhase == .exploded {
                    CrashExplosionView(intensity: min(1, crashProgress * 1.15), size: 148)
                ...
                if crashPhase == .none || crashPhase == .falling {
                    CrashRocketView(
                        angle: rocketAngle,
                        scale: phase == .running ? 1.08 : 0.94,
                        thrust: showExhaust,
                        exhaustIntensity: exhaustIntensity,
                        crashed: crashPhase == .falling,
                        impactFlash: crashPhase == .falling && crashProgress > 0.9
                    )
                    .modifier(RocketFlightMotionModifier(active: showExhaust))
                    .position(rocketPos)
                }
    ...
    private func chartChromeOverlay(width: CGFloat, height: CGFloat) -> some View {
        ZStack {
            ChartEntriesOverlay(
                ...
                lastActionMessage: entriesActionMessage
            )
        ...
        VStack(alignment: .trailing, spacing: 5) {
            ...
            if phase == .betting {
                Text("ENTRY WINDOW")
                    ...
                    .tracking(1.1)
                    .foregroundStyle(NFGTheme.muted.opacity(0.9))
                    .padding(.top, 2)
                    .chartOverlayTextLegibility()
    ...
    @ViewBuilder
    private func chartParticleLayer(
        rocketPos: CGPoint,
        exhaustAngle: CGFloat,
        showExhaust: Bool,
        exhaustIntensity: CGFloat,
        impactPoint: CGPoint,
        impactBurstToken: Int
    ) -> some View {
        ZStack {
            ...
            RocketFallTrailParticlesView(
                rocketPosition: rocketPos,
                isActive: crashPhase == .falling
            )
            if crashPhase == .exploded {
    ...
    private func chartBackground(phase: GamePhase) -> some View {
        ZStack {
            CrashSceneBackground(phase: phase)
            if phase == .running {
                RadialGradient(
                    colors: [NFGTheme.accent.opacity(0.08), .clear],
                ...
                RadialGradient(
                    colors: [NFGTheme.danger.opacity(0.28), .clear],
                    center: .bottomLeading,
                    startRadius: 0,
                    endRadius: 280
                )
    ...
    /// Large flame at 1×; tapers as multiplier climbs during the round.
    private func exhaustIntensity(for multiplier: Double) -> CGFloat {
        guard multiplier >= 1 else { return 1.15 }
        let t = min(1, max(0, log(max(1, multiplier)) / log(40)))
        return CGFloat(1.2 - 0.78 * t)
    }
